...
class PerpetualMarketGridStrategy(StrategyPyBase):
    ...
    # jk
        # self._price_floor = 100
        # self._price_ceiling = 110
        # self._order_levels = 10
        # interval = 0
        # last_step = self._price_floor
        # for level in range(1, self._order_levels):
        #         next_step = self._price_floor + (
        #                 level * (self._price_ceiling - self._price_floor) / self._order_levels
        #         )
        #         interval = interval + (next_step - last_step) / last_step
        #         last_step = next_step
        #
        # self._interval = interval / self._order_levels
        # tmp = round(self._interval * 100, 3)
        # self.logger().info(f"interval calculated as ")
    ...
    def format_status(self) -> str:
        ...
        if len(self.active_orders) > 0:
            ...
            lines.extend(["", "  No active grid orders."])
    ...
    def tick(self, timestamp: float):
        ...
        try:
            if not self._all_markets_ready:
                ...
                if not self._all_markets_ready:
                    # M({self.trading_pair}) Grid sell order {order_id}arkets not ready yet. Don't do anything.
                    if should_report_warnings:
                        self.logger().warning("Markets are not ready. No market grid trades are permitted.")
                    return
            ...
            if should_report_warnings:
                if not all([market.network_status is NetworkStatus.CONNECTED for market in self.active_markets]):
                    self.logger().warning("WARNING: Some markets are not connected or are down at the moment. Market "
                                          "grid may be dangerous when markets or networks are unstable.")
    ...
    def did_fill_order(self, order_filled_event: OrderFilledEvent):
        ...
        if market_info is not None:
            if self._logging_options & self.OPTION_LOG_GRID_ORDER_FILLED:
                self.log_with_clock(
                    logging.INFO,
                    f"({market_info.trading_pair}) Grid "
    ...
    def did_complete_buy_order(self, order_completed_event: BuyOrderCompletedEvent):
        ...
        self.log_with_clock(
            logging.INFO,
            f"({self.trading_pair}) Grid buy order {order_id} "
            f"({limit_order_record.quantity} {limit_order_record.base_currency} @ "
            f"{limit_order_record.price} {limit_order_record.quote_currency}) has been completely filled."
        )
        self.notify_hb_app_with_timestamp(
            f"Grid BUY order {limit_order_record.quantity} {limit_order_record.base_currency} @ "
    ...
    def did_complete_sell_order(self, order_completed_event: SellOrderCompletedEvent):
        ...
        self.log_with_clock(
            logging.INFO,
            f"({self.trading_pair}) Grid sell order {order_id} "
            f"({limit_order_record.quantity} {limit_order_record.base_currency} @ "
            f"{limit_order_record.price} {limit_order_record.quote_currency}) has been completely filled."
        )
        self.notify_hb_app_with_timestamp(
            f"Grid SELL order {limit_order_record.quantity} {limit_order_record.base_currency} @ "
            f"{limit_order_record.price} {limit_order_record.quote_currency} is filled."
        )
